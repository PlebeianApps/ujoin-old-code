        ...
        <div id="contentRight">
        	...
        	<div id="contentRightTop">
                ...
                <div id="main">
					...
					<div id="billTabs">
                        <?PHP
                        	$sql = "SELECT * FROM $pn_tables->form_letter WHERE status = 1 AND type = 'letter' ORDER BY id DESC;";
                        	$data = $wpdb->get_results($sql, ARRAY_A);
                        ?>
                        <div class="tabHeader">Public Letters to the Editor</div>
                        <div class="tabContent">
                            <table cellpadding="0" cellspacing="0" border="0" class="display tablageneral advocacy" id="tablaUno">
                                ...
                                ?>
                                    <tr>
                                        <td  class="tdTitle"><a href="<?php bloginfo('url');?>/?p=208&letter=<?=$key['id'];?>"><?=$key['title'];?></a></td>
                                        <?PHP
						  	          		$uid = $key['wuid'];
						  	          		$sql = "SELECT organization FROM $pn_tables->user_profile WHERE wuid = $uid;";
		                    				$group = $wpdb->get_var($sql);						  	          		
						            	?>
                                        <td  class="tdBy"><a href="<?php bloginfo('url');?>/?p=208&letter=<?=$key['id'];?>"><?=$group;?></a></td>
                        ...
                        <div class="tabHeader">My Group Letters to the Editor</div>
<?PHP
						$uid = get_user_group();
						//%d %c %Y %h %i %p
						$sql = "SELECT * FROM $pn_tables->form_letter
								WHERE wuid = $uid AND type = 'letter' AND status = 1;";
						$data = $wpdb->get_results($sql, ARRAY_A);
?>   
                        <div class="tabContent">
