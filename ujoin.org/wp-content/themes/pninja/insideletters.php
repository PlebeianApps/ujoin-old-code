<?php
/*
Template Name: Inside Letters to the Editor
*/
?>
<?PHP
	global $pn_tables;
	$form_table = $pn_tables->form_letter;
	$form_var = 'letter';
	$form_query = array();
...
<script type="text/javascript" src="<?php bloginfo('stylesheet_directory') ?>/js/letter.js"></script>
<script type="text/javascript">
...
<div id="content-page">              
		...
		<div id="contentLeft">
            ...
            <?PHP if(trim($billName)): ?>
            	<li style="display:none;"><a href="#" id="addUpdate">Add / Update Bill Path</a></li>
           <?PHP endif;?>
            	<li><a href="#" id="editPage">Edit Letters to the Editor</a></li>
            	<li><a href="#" id="deleteLetter">Delete Letters to the Editor</a></li>
              <?PHP if(!testimony_exists($form_query['bill'])):?>
              	<li><a href="<?=home_url('/');?>?page_id=191&template=letter&template_id=<?=$letter?>">Duplicate this to: Testimony Action Page</a></li> 
              <?PHP endif; ?>
              <?PHP if(!advocacy_exists($form_query['bill'])):?>
              	<li><a href="<?=home_url('/');?>?page_id=176&template=letter&template_id=<?=$letter?>">Duplicate this to: Advocacy Action Page</a></li> 
              <?PHP endif; ?>
              <?PHP if(!letter_exists($form_query['bill'])):?>
              	<li><a href="<?=home_url('/');?>?page_id=172&template=letter&template_id=<?=$letter?>">Duplicate this to: Letter Action Page</a></li> 
              <?PHP endif; ?>
              <?PHP if(!petition_exists($form_query['bill'])):?>
              	<li><a href="<?=home_url('/');?>?page_id=178&template=letter&template_id=<?=$letter?>">Duplicate this to: Petition Action Page</a></li>
              <?PHP endif; ?> 
              <?PHP else:?>
              	<li><a href="#">
                	<h6>Support Video</h6>
        	...
        	<div id="contentRightTop">
                ...
                <div id="main">
                  ...
                  <ul id="newBill" style="padding:4px 0 0;">
                        ...
                        <li><label class="labelLeft">Overview</label><input <?=get_form_value($form_query, 'overview');?> id="f-overview" type="text" size="40" class="text"></li>
                        <li><label class="labelLeft">Talking Points</label><textarea id="f-points" rows="5" cols="40" class="uniform"><?=$form_query['talking_points']?></textarea></li>
                        <li><label class="labelLeft">Sample Message</label><textarea id="f-message" rows="5" cols="40" class="uniform"><?=$form_query['message']?></textarea></li>
                        <li><label class="labelLeft">Public Action Page</label>
                       	...
                       	<li style="display:none;"><label class="labelLeft">Users can post video?</label>
                       		<select id="f-nimbb">
...
<?PHP
						$sql = "SELECT id, CONCAT_WS(' ', first_name, last_name)AS name, email, published FROM $pn_tables->form_user WHERE parent_table = 'pn_form_letter' AND parent = ".$letter;
...
?>
                        ...
                        <?PHP foreach($letters as $key):?>
							<a href="<?php bloginfo('url');?>/?p=222&letter=<?=$key['id'];?>" class="tabContentRow">
                    ...
                    <script type="text/javascript">
                    	$(document).ready(function(){
                    		$('#export-testimony').click(function(){
                    			var url = getURL("exportPage", [_letter,'pn_form_letter']);
...
<div id="content-page">              
		...
		<div id="contentLeft">
            ...
            <span class="contentLeftTitle"><span><?=$form_query['title'];?></span></span>
            <ul>
            <?PHP if(trim($form_query['youtube']) != ''):?>
            	<!--<li><a href="#">
                	<h6>Support Video</h6>
                </a></li>  
        ...
        <div id="contentRight">
        	...
        	<div id="contentRightTop">
                ...
                <div id="main">
                    ...
                    <h1 style="margin:0;">Your letter to the editor</h1>                                               
    ...
    theme : 'clean'
                    ...
                    <form id="g-form">
                      <ul id="newBill" style="background:none; margin-top:-28px 10px 0 0;">
                      	<li><label class="labelLeft">Email Subject</label><input id="g-subject" type="text" size="40" class="text"></li>
                        ...
                        <li><label class="labelLeft">File</label><input id="g-file" type="file" size="40"/></li>
                        <li><label class="labelLeft">Your letter</label><textarea id="g-testimony" cols="40" rows="5"><?=$form_query['message'];?></textarea></li>
...
<!--                        <li><p style="line-height: 150%; padding: 1em; font-size: .8em;">
                ...
                If the words displayed are too difficult to read click <a href="javascript:Recaptcha.reload();">here</a> or on the recycle arrows to get another set of words. 
                    ...
                    <?PHP
                    	$sql = "SELECT COUNT(id)AS total FROM $pn_tables->form_user WHERE parent_table = 'pn_form_letter' AND parent = ".$letter;
