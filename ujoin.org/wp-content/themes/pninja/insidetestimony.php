...
<div id="content-page">              
 ...
 <span class='st_facebook_large' st_url='<?= "http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"] ?>' displayText='Share it'></span>
...
/*
                    $(function(){
                      $('#g-file').uploadify({
                        langFile : '<?php bloginfo('stylesheet_directory'); ?>/js/uploadify/uploadifyLang_en.js',
                        swf      : '<?php bloginfo('stylesheet_directory'); ?>/js/uploadify/uploadify.swf',
                        uploader : '<?php bloginfo('url');?>/?ajax',
                        method   : 'post',
                        fileSizeLimit : 7*1024, // 7 MB
                        multi    : false,
                        onUploadSuccess : function(_, _, $data) {
                            if($data.hasOwnProperty('recaptchaError')) {
                               $('#loading-div').remove();
                               $('#g-form ul, #g-submit, #uniform-g-submit').show();
                               alert($data.recaptchaError);
                            } else {
                            $('#loading-div').html('<p>Thank you</p>');
                            }
                          },
                      })
                    });
