...
function init_pn_plugin () {	
	...
	if (isset($_POST['edit'])) {
		...
		//print_r(stripslashes($_POST['votes']));
		...
		$wpdb->query("DELETE FROM $pn_tables->legs2bills WHERE bill_id = '".$_POST['bill_id']."';");
		...
		foreach($bupd as $key1=>$value1) {
			...
			foreach($value1->legsvotes as $key2=>$value2) {
				...
				$wpdb->query("INSERT INTO $pn_tables->legs2bills 
								VALUES('".$_POST['bill_id']."', '".$key2."', '".$key1."', '".$value2."')");			
			}			
		...
		for($x=0; $x<count($d); $x++) {
			...
			$wpdb->query("DELETE FROM $pn_tables->legs2bills 
						WHERE bill_id = '".$_POST['bill_id']."' AND comm_id = '".$d[$x]->comm_id."' AND crp_id = '".$d[$x]->crp_id."';");
			
			if ($d[$x]->vote != 0) {
				$wpdb->query("INSERT INTO $pn_tables->legs2bills 
								VALUES('".$_POST['bill_id']."', '".$d[$x]->crp_id."', '".$d[$x]->comm_id."', '".$d[$x]->vote."')");
			}
		}
		*/
	...
	foreach ($result as $data) {	
		...
		$result_i = $wpdb->get_results("SELECT crp_id, CONCAT(firstname, ' ', lastname) AS lname FROM $pn_tables->legislators WHERE crp_id IN (".join(",", $c).");");
		...
		$result_r = $wpdb->get_results("SELECT * FROM $pn_tables->bills_updates WHERE bill_id = '".$data->id."';", ARRAY_A);
	...
	foreach ($result_comm as $data_comm) {
		$arr_comm[] = "<option value='".$data_comm->comm_id."'>".$data_comm->name."</option>";
	}
